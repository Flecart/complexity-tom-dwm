...
def prompt_and_compare_line(prompt: list[str], start_index: int, offset: int):
    ...
    Args:
        ...
        offset: The offset to be used.
    """
    text_prompt = ". ".join(prompt[start_index:start_index+offset])
    next_part = ". ".join(prompt[start_index+offset:])
    small_prompt = text_prompt + ". " # this new line is important somehow.
    response = queryGPT35Instruct(small_prompt)
    output = response["choices"][0]["text"]
